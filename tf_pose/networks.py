...
def get_network(type, placeholder_input, sess_for_load=None, trainable=True):
    ...
    if sess_for_load is not None:
        if type == 'cmu' or type == 'vgg':
            ...
            ckpts = {
                'mobilenet': '/data/models/trained/mobilenet_%s/model-246038' % s,
                'mobilenet_thin': '/data/models/trained/mobilenet_thin_%s/model-449003' % s,
                'mobilenet_fast': '/data/models/trained/mobilenet_fast_%s/model-189000' % s,
                'mobilenet_accurate': './data/models/trained/mobilenet_accurate/model-170000'
            }
...
def get_graph_path(model_name):
    ...
    if os.path.exists(os.path.join(base_data_dir, 'data/models')):
        base_data_dir = os.path.join(base_data_dir, 'data/models')
    else:
        base_data_dir = os.path.join(base_data_dir, 'data/models')
